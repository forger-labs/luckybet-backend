...
export class PlayerChestsCore implements ForManagePlayerChests {
  ...
  async getChestProgressById(
    ...
    const { startDate, endDate, periodKey } = this.getPeriodRange(
      chest.periodType,
    );
  ...
  async joinChest(
    ...
    if (!created) {
      ...
      throw new ConflictException(
        'Error al registrar participación en el cofre',
      );
    }
  ...
  async listPlayerChests(
    ...
    const [claims, total] = await this.claimRepo.getPlayerChests(
      playerId,
      filter,
    );
    return {
  ...
  async getPlayerChestsProgress(
    ...
    for (const chest of activeChests) {
      ...
      const completedMissions =
        await this.userMissionRepo.countCompletedBetween(
          playerId,
          startDate,
          endDate,
        );
  ...
  async claimChest(
    ...
    if (locked) {
      if (locked.status === RewardStatus.CLAIMED) {
        throw new BadRequestException(
          'Ya has reclamado este cofre en el periodo actual',
        );
      }
      ...
      if (locked.status === RewardStatus.PROCESSING) {
        throw new ConflictException(
          'El cofre ya está siendo procesado en este momento',
        );
      }
      locked = await this.claimRepo.updateStatus(
        locked.id,
    ...
    if (!locked) {
      throw new ConflictException(
        'El cofre ya está siendo procesado o fue reclamado',
      );
    }
    ...
    if (!mutationResult.success) {
      ...
      return await this.claimRepo.updateStatus(
        ...
        {
          errorMessage:
            mutationResult.errorMessage ?? 'Error al acreditar saldo de cofre',
        },
      );
    }
  ...
  async resolveUncertainClaim(
    ...
    if (!claim) {
      throw new NotFoundException(
        `Reclamo de cofre con ID ${claimId} no encontrado`,
      );
    }
    ...
    if (action === 'RESOLVE_CLAIMED') {
      return await this.claimRepo.updateStatus(claim.id, RewardStatus.CLAIMED, {
        ...
        externalOperationId:
          options?.externalOperationId ?? claim.externalOperationId,
        claimedAt: new Date(),
      });
    }
    ...
    throw new BadRequestException(
      `Acción de resolución desconocida: ${action}`,
    );
  }
