...
export const gamePlayStepConfigSchema = z
	.object({
	...
	.refine(data => !(data.provider && data.gameId), {
		message:
			'No se pueden especificar provider y gameId simultáneamente en un paso de juego',
		path: ['gameId'],
	})
	.refine(data => !(data.gameId && data.minUniqueGames && data.minUniqueGames > 1), {
		message: 'Si se especifica un gameId, minUniqueGames no puede ser mayor a 1',
		path: ['minUniqueGames'],
	});
