...
export class AdminPanelService implements ForAdminPanel {
  ...
  async ensureSession(): Promise<string> {
    const cachedSession = await this.cache.get<string>(
      LUCKYBET_ADMIN_SESSION_CACHE_KEY,
    );
    if (cachedSession) {
  ...
  private async requestWithSession<T>(
    ...
    try {
      ...
      if (
        ...
        this.logger.warn(
          'Sesión expirada detectada por error HTTP. Reintentando...',
        );
        await this.invalidateSession();
        sessionId = await this.ensureSession();
        const retryResponse = await requestFn(sessionId);
  ...
  async searchPlayer(
    searchLogin: string,
    page = 1,
  ): Promise<LuckyBetSearchUser[]> {
    const url = `${this.panelHost}/index.php?act=admin&area=search&response=js`;

    const data = await this.requestWithSession<LuckyBetSearchResponse>(
      (sessionId) => {
        const params = new URLSearchParams();
        params.append('search_login', searchLogin);
        params.append('page', String(page));

        return this.client.post<LuckyBetSearchResponse>(
          url,
          params.toString(),
          {
            headers: {
  ...
  async getPlayerBalance(
    ...
    return await this.requestWithSession<LuckyBetBalanceResponse>(
      (sessionId: string) => {
  ...
  async getPlayerHistory(
    ...
    return await this.requestWithSession<LuckyBetHistoryResponse>(
      (sessionId: string) => {
  ...
  async creditPlayer(
    ...
    const data = await this.requestWithSession<Record<string, unknown>>(
      (sessionId) => {
  ...
  async getLastPlayedGames(
    ...
    const gameId = options?.gameName;
    const minBet = options?.minBet;
    ...
    if (sessions.length > 0) {
      for (const item of sessions) {
        ...
        if (
          gameId !== undefined &&
          gameId.toLowerCase() !== gameIdentifier.toLowerCase()
        ) {
          continue;
        }

        if (item.bet !== undefined && minBet !== undefined) {
          const bet = Number(item.bet);
          if (Number.isNaN(bet) || bet < minBet) {
            continue;
          }
        }
        ...
        if (!deduplicatedGames.has(rawId)) {
          deduplicatedGames.set(rawId, {
            gameId: rawId,
            gameName: item.game_name || this.formatGameName(rawId),
            lastPlayedAt,
